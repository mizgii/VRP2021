class DataModel():
	...
	def __init__(self, n,nv,cap):
		...
		with open('dist_mx', 'rb') as f:
			distmat = np.array(pickle.load(f))
		random_indices = np.concatenate((np.array([0]), np.random.choice(np.arange(1, 51), n)))
		distmat = distmat[random_indices][:, random_indices].tolist()
